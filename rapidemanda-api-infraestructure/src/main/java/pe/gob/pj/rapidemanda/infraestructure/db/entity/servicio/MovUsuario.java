...
@Table(name = "MOV_USUARIO", schema = ProjectConstants.Esquema.RAPIDEMANDA)
@NamedQueries(value = { @NamedQuery(name = MovUsuario.Q_ALL, query = "SELECT mu FROM MovUsuario mu") })
@FilterDefs(value = {
		@FilterDef(name = MovUsuario.F_ACCESO, parameters = {
				@ParamDef(name = MovUsuario.P_ACTIVO, type = String.class),
				@ParamDef(name = MovUsuario.P_USUARIO, type = String.class) }),
		@FilterDef(name = MovUsuario.F_ID, parameters = { @ParamDef(name = MovUsuario.P_ID, type = Integer.class) }),
		@FilterDef(name = MovUsuario.F_USUARIO, parameters = {
				@ParamDef(name = MovUsuario.P_USUARIO, type = String.class) }) })
@Filters(value = {
		@Filter(name = MovUsuario.F_ACCESO, condition = "L_ACTIVO=:" + MovUsuario.P_ACTIVO + " AND X_USUARIO=:"
				+ MovUsuario.P_USUARIO),
...
public class MovUsuario extends AuditoriaEntity implements Serializable {
	...
	*/
	...
	@Id
	@SequenceGenerator(name = "SEQ_MOV_USUARIO", schema = ProjectConstants.Esquema.RAPIDEMANDA, sequenceName = "USEQ_MOV_USUARIO", initialValue = 1, allocationSize = 1)
