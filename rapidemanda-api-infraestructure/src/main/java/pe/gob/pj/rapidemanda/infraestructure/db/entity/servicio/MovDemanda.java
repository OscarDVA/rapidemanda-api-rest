...
@Filters(value = { @Filter(name = MovDemanda.F_ID, condition = "N_DEMANDA=:" + MovDemanda.P_ID),
		...
		@Filter(name = MovDemanda.F_ESTADO_DEMANDA_MULTI, condition = "B_ESTADO_DEMANDA in (:"
				+ MovDemanda.P_ESTADO_DEMANDA_LIST + ")"),
		@Filter(name = MovDemanda.F_USUARIO, condition = "N_USUARIO=:" + MovDemanda.P_USUARIO),
		@Filter(name = MovDemanda.F_TIPO_PRESENTACION, condition = "B_TIPO_PRESENTACION=:"
				+ MovDemanda.P_TIPO_PRESENTACION),
		@Filter(name = MovDemanda.F_TIPO_RECEPCION, condition = "C_TIPO_RECEPCION=:" + MovDemanda.P_TIPO_RECEPCION),
		@Filter(name = MovDemanda.F_USUARIO_RECEPCION, condition = "N_USUARIO_RECEPCION=:"
				+ MovDemanda.P_USUARIO_RECEPCION),
		@Filter(name = MovDemanda.F_FECHA_COMPLETADO_RANGO, condition = "F_COMPLETADO >= :"
				+ MovDemanda.P_FECHA_COMPLETADO_INICIO + " and F_COMPLETADO <= :"
				+ MovDemanda.P_FECHA_COMPLETADO_FIN) })
...
public class MovDemanda extends AuditoriaEntity implements Serializable {
	...
	// Nuevo campo: indica si la demanda tiene representante
	@Column(name = "L_TIENE_REPRESENTANTE", nullable = false, length = 2)
	private String tieneRepresentante;
